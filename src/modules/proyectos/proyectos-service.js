...
(function exposeProyectosService(global) {
  ...
  const service = Object.freeze({
    listar(options = {}) {
      return requireIpcService().call("proyectos", "listar", normalizeListOptions(options));
    },
    obtener(projectId) {
      return requireIpcService().call("proyectos", "obtener", projectId);
    },
    crear(payload) {
      return requireIpcService().call("proyectos", "crear", normalizeProjectPayload(payload));
    },
    ...
    importarArchivos(projectId) {
      return requireIpcService().call("archivos", "importar", projectId);
    },
    abrirArchivo(fileId) {
      return requireIpcService().call("archivos", "abrir", fileId);
    },
    mostrarArchivoEnCarpeta(fileId) {
      return requireIpcService().call("archivos", "mostrarEnCarpeta", fileId);
    },
    abrirCarpetaDelProyecto(projectId) {
      return requireIpcService().call("archivos", "abrirCarpetaProyecto", projectId);
    },
